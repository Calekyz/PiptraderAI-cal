...
import { GoogleSignInPromptModal } from './GoogleSignInPromptModal';
...
export const PipNexAuthCard: React.FC<PipNexAuthCardProps> = ({ onSuccessAuth, initialMode }) => {
  ...
  // 6-Digit Email Verification Flow State
  ...
  // Google direct sign in trigger
  const handleGoogleSignInDirect = async () => {
    setErrorMessage(null);
    try {
      const launched = await triggerGoogleOAuth({
        onSuccess: (profile) => {
  ...
  // Submit Handler with strict authentication & 6-digit email verification enforcement
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (authMode === 'signup') {
      ...
      try {
        ...
        if (err.requireVerification) {
          setVerificationEmail(err.email || email.trim().toLowerCase());
          setVerificationNotice(err.message || 'Please verify your email address to continue.');
          setShowVerificationStep(true);
        } else {
    ...
    <div className="relative w-full max-w-[500px] mx-auto z-20">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isDevelopmentMode() && (
            <button
              id="continue-as-dev-btn"
              type="button"
              onClick={() => {
                const user = enterAsDevUser();
                onSuccessAuth(user);
              }}
              className="w-full mt-2 py-3 px-4 rounded-xl bg-[#1a2438] hover:bg-[#22304a] border border-cyan-500/30 hover:border-cyan-400/50 text-cyan-300 text-sm font-semibold tracking-wide transition-all duration-200 cursor-pointer"
            >
              Continue as Dev
            </button>
          )}
